  ...
  with BeforeAll
{
  ...
  if(new File("test/pdbench").exists()){
    ...
    skipped("Skipping RepairKey timing tests (Run `sbt datasets` to download required data)")
  }
  ...
  def createKeyRepairLens(tableFields:(String, String, Type, Int), tableSuffix: String = "_cleaned") =  
  {
    ...
    s"Create Key Repair Lens for table: $baseTable" >> {
      if(!db.tableExists(baseTable)){
        ...
        LoadCSV.handleLoadTable(db, 
          baseTable, 
          new File(s"test/maybms/${baseTable}.tbl"), 
          Map(
            "HEADER" -> "NO",
            "DELIMITER" -> "|"
          )
      ...
      if(query(s"SELECT NAME FROM MIMIR_VIEWS WHERE NAME = '${testTable}").allRows.isEmpty){
