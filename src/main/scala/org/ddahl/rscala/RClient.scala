...
object RClient {
  ...
  /** Returns an instance of the [[RClient]] class, using the path to the executable specified by `rCmd`.
    ...
    * {{{val R = org.ddahl.rscala.RClient()}}}
    */
  def apply(rCmd: String = defaultRCmd, port: Int = 0, debug: Boolean = false, referenceMap:HashMap[Int, (Any,String)] = null): RClient = {
    ...
    val server = new Server(null, sockets, referenceMap, conduit, out, in, debugger, false, prntWrtr, null)
