  ...
  with ModelCache
{
  ...
  def guessSparkModelType(t:Type) : String = {
    t match {
      case TFloat() if db.schemaOf(query).length == 1 => "Regression"
  ...
  private def classify(rowid: RowIdPrimitive, rowValueHints: Seq[PrimitiveValue]): Seq[(String,Double)] = {
     {if(rowValueHints.isEmpty){
       ...
       sparkMLInstance.extractPredictions(learner.get,
           sparkMLInstance.applyModel(learner.get,  ("rowid", TString()) +: db.schemaOf(query).filterNot(_._1.equalsIgnoreCase("rowid")), List(List(rowid, rowValueHints(colIdx)))))
     }} match {
         case Seq() => Seq()
         case x => x.unzip._2
       }
