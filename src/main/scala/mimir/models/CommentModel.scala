  ...
  with FiniteDiscreteDomain
{
  ...
  def argTypes(idx: Int) = colTypes 
  def varType(idx: Int, args: Seq[Type]) = args(idx)
  def bestGuess(idx: Int, args: Seq[PrimitiveValue], hints: Seq[PrimitiveValue]  ) = args(idx)
  def sample(idx: Int, randomness: Random, args: Seq[PrimitiveValue], hints: Seq[PrimitiveValue]) = args(idx)
  def reason(idx: Int, args: Seq[PrimitiveValue],hints: Seq[PrimitiveValue]): String = comments(idx)
  ...
  def getDomain(idx: Int, args: Seq[PrimitiveValue], hints:Seq[PrimitiveValue]): Seq[(PrimitiveValue,Double)] = Seq((args(idx), 0.0))
