    ...
    cols.map( (col) => {
      ...
      val model =
        db.models.getOption(modelName) match {
          ...
          case None => {
            if (randomParameter == false) {
              val model = new FuncDepModel(modelName, col, query)
              model.train(db)
...
@SerialVersionUID(1000L)
class FuncDepModel(name: String, colName: String, query: Operator)
  extends Model(name)
  with NeedsReconnectToDatabase
{
  var numSamples = 0
