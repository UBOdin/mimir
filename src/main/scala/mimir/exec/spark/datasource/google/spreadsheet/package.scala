 ...
 */
package mimir.exec.spark.datasource.google
...
package object spreadsheet {
  ...
  implicit class SpreadsheetDataFrameReader(reader: DataFrameReader) {
    def spreadsheet: String => DataFrame =
      reader.format("mimir.exec.spark.datasource.google.spreadsheet").load
  }
  ...
  implicit class SpreadsheetDataFrameWriter[T](writer: DataFrameWriter[T]) {
    def spreadsheet: String => Unit =
      writer.format("mimir.exec.spark.datasource.google.spreadsheet").save
  }
}