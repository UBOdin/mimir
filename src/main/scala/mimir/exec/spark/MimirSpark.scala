  ...
  extends LazyLogging
{
  ...
  def init(config: MimirConfig){
    ...
    if(remoteSpark){ 
      ...
      HadoopUtils.writeToHDFS(sparkCtx, "sparsity_2.11-1.4.jar", new File(s"${System.getProperty("user.home")}/.ivy2/cache/info.mimirdb/sparsity_2.11/jars/sparsity_2.11-1.4.jar"), overwriteJars)
      ...
      sparkCtx.addJar(s"$hdfsHome/sparsity_2.11-1.4.jar")
