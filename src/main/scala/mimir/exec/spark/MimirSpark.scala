  ...
  extends LazyLogging
{
  ...
  def init(config: MimirConfig){
    ...
    if(remoteSpark || !localSpark){ 
      ...
      HadoopUtils.writeToHDFS(sparkCtx, s"sparsity_${scalaVersion}-1.6.1.jar", new File(getJarPath("info.mimirdb", "sparsity", "1.6.1", scalaVersion)), overwriteJars)
      ...
      sparkCtx.addJar(s"${hdfsPath}sparsity_${scalaVersion}-1.6.1.jar")
