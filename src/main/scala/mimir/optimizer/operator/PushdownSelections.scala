...
object PushdownSelections extends OperatorOptimization {
	...
	def apply(o: Operator): Operator = 
	{
		o match {
			...
			//TODO: mike: i am going to finish this after cidr
			//  it is almost there but still a few things that should use 
			//  code that is already implemented in ExpressionUtils 
			...
			case Select(cond, LensView(lens, table, query, annotations)) => 
				Select(cond, LensView(lens, table, apply(query), annotations))
